...
import adris.altoclef.ItemInfo.ToolDurabilityMapping;
import adris.altoclef.ItemInfo.ArmorSlotUtil;
...
public class StorageHelper {
    ...
    public static ItemStack getItemStackInSlot(Slot slot) {
        ...
        if (inv != null) {
            if (slot.equals(PlayerSlot.OFFHAND_SLOT))
                return inv.getStack(PlayerSlot.OFFHAND_SLOT.getInventorySlot());
            if (slot.equals(PlayerSlot.ARMOR_HELMET_SLOT))
                return inv.getStack(PlayerSlot.ARMOR_HELMET_SLOT.getInventorySlot());
            if (slot.equals(PlayerSlot.ARMOR_CHESTPLATE_SLOT.getInventorySlot()))
                return inv.getStack(PlayerSlot.ARMOR_CHESTPLATE_SLOT.getInventorySlot());
            if (slot.equals(PlayerSlot.ARMOR_LEGGINGS_SLOT))
                return inv.getStack(PlayerSlot.ARMOR_LEGGINGS_SLOT.getInventorySlot());
            if (slot.equals(PlayerSlot.ARMOR_BOOTS_SLOT))
                return inv.getStack(PlayerSlot.ARMOR_BOOTS_SLOT.getInventorySlot());
        }
        try {
    ...
    public static boolean isArmorEquipped(AltoClef mod, Item... any) {
        for (Item item : any) {
            boolean ISArmorItem = Registries.ITEM.getEntry(item).isIn(ItemTags.ARMOR_ENCHANTABLE);
            if (ISArmorItem) {
                ItemStack equippedStack = mod.getPlayer().getEquippedStack(ArmorSlotUtil.getArmorSlot(item));
