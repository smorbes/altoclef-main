    ...
    public static Item bestWeapon(AltoClef mod) {
        ...
        if (!invStacks.isEmpty()) {
            ...
            for (ItemStack invStack : invStacks) {
                boolean IsHanditemSword = Registries.ITEM.getEntry(invStack.getItem()).isIn(ItemTags.SWORDS);
                if (IsHanditemSword) {
                    float itemDamage = SwordDamageUtility.getSwordDamage(invStack.getItem().getName().getString().toLowerCase());
                    Item handItem = StorageHelper.getItemStackInSlot(PlayerSlot.getEquipSlot()).getItem();
                    boolean IsSecondHanditemSword = Registries.ITEM.getEntry(handItem).isIn(ItemTags.SWORDS);
                    if (IsSecondHanditemSword) {
                        handDamage = SwordDamageUtility.getSwordDamage(handItem.getName().getString().toLowerCase());
                    }
