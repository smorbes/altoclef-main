...
import adris.altoclef.ItemInfo.PickaxeMiningSpeedUtility;
        ...
        if (_cursorStackTimer.elapsed() && !mod.getFoodChain().needsToEat()) {
            ...
            if (cursorStack != null && !cursorStack.isEmpty()) {
                ...
                if (cursorStack.isSuitableFor(mod.getWorld().getBlockState(_subtask.miningPos()))) {
                    ...
                    boolean IsMiningItem = Registries.ITEM.getEntry(cursorStack.getItem()).isIn(ItemTags.MINING_ENCHANTABLE);
                    if (IsMiningItem) {
                        boolean IsCurrentlyEquippedMiningItem = Registries.ITEM.getEntry(currentlyEquipped).isIn(ItemTags.MINING_ENCHANTABLE);
                        if (IsCurrentlyEquippedMiningItem) {
                            Item swapPick = cursorStack.getItem();
                            if (PickaxeMiningSpeedUtility.getMiningSpeed(swapPick.getName().toString().toLowerCase()) > PickaxeMiningSpeedUtility.getMiningSpeed(currentlyEquipped.getName().toString().toLowerCase())) {
