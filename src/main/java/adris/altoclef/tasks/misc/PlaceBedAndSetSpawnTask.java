    ...
    protected Task onTick(AltoClef mod) {
        ...
        if (nearestBed.isPresent() && !WorldHelper.isAir(mod, nearestBed.get())) {
            ...
            if (nearestBed.get().isWithinDistance(mod.getPlayer().getPos(), 40)) {
                ...
                List<HostileEntity> list = mod.getWorld().getEntitiesByClass(HostileEntity.class, new Box(vec3d.getX() - 8.0, vec3d.getY() - 5.0, vec3d.getZ() - 8.0, vec3d.getX() + 8.0, vec3d.getY() + 5.0, vec3d.getZ() + 8.0), (entity) -> entity.isAngryAt(serverWorld, mod.getPlayer()));
                if (!list.isEmpty()) {
