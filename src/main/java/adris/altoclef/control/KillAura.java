...
import adris.altoclef.ItemInfo.SwordDamageUtility;
...
public class KillAura {
    ...
    public static void equipWeapon(AltoClef mod) {
        ...
        if (!invStacks.isEmpty()) {
            ...
            for (ItemStack invStack : invStacks) {
                boolean isSword = Registries.ITEM.getEntry(invStack.getItem()).isIn(ItemTags.SWORDS);
                if (isSword) {
                    // Get the sword name in lowercase and use it to get the damage from SwordDamage class
                    String swordName = invStack.getItem().getTranslationKey().toLowerCase();  // Assuming the translation key is used for the name
                    float itemDamage = SwordDamageUtility.getSwordDamage(swordName);

                    // Get the damage of the current equipped item
                    Item handItem = StorageHelper.getItemStackInSlot(PlayerSlot.getEquipSlot()).getItem();
                    boolean IsHanditemSword = Registries.ITEM.getEntry(handItem).isIn(ItemTags.SWORDS);
                    if (IsHanditemSword) {
                        String handSwordName = handItem.getTranslationKey().toLowerCase();
                        handDamage = SwordDamageUtility.getSwordDamage(handSwordName);
                    }

                    // Equip the item with higher damage
    ...
    public void tickEnd(AltoClef mod) {
        ...
        if (entities.isPresent() && mod.getPlayer().getHealth() >= 10 &&
            ...
            ItemStack offhandItemStack = new ItemStack(offhandItem);
